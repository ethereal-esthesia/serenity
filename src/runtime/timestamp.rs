...
#[cfg(test)]
mod tests {
    use super::{INPUT_TIMESTAMP_PAYLOAD_MASK, InputTimestamp};

    #[test]
    fn input_timestamp_masks_top_bit_on_from_raw() {
        let raw = u64::MAX;
        let ts = InputTimestamp::from_raw(raw);
        assert_eq!(ts.raw(), INPUT_TIMESTAMP_PAYLOAD_MASK);
    }

    #[test]
    fn input_timestamp_wrapping_add_rolls_at_payload_boundary() {
        let near_wrap = InputTimestamp::from_raw(INPUT_TIMESTAMP_PAYLOAD_MASK - 2);
    ...
    fn input_timestamp_wrapping_delta_handles_rollover() {
        let earlier = InputTimestamp::from_raw(INPUT_TIMESTAMP_PAYLOAD_MASK - 5);
        let later = InputTimestamp::from_raw(3);
        assert_eq!(later.wrapping_delta_since(earlier), 9);
    }

    #[test]
    fn input_timestamp_wrapping_delta_zero_for_same_value() {
        let ts = InputTimestamp::from_raw(123_456);
